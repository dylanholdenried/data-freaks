-- Upload history / unwind: link imported deals to batch rows + unwind RPC

alter table public.deal_import_rows
  add column if not exists deal_id uuid references public.deals(id) on delete set null;

create index if not exists deal_import_rows_deal_id_idx
  on public.deal_import_rows(deal_id);

alter table public.deal_import_batches
  add column if not exists unwound_at timestamptz;

alter table public.deal_import_batches
  drop constraint if exists deal_import_batches_status_check;

alter table public.deal_import_batches
  add constraint deal_import_batches_status_check
  check (status in ('pending_review', 'committed', 'cancelled', 'unwound'));
...
-- Unwind a committed batch: delete linked deals (cascade children), mark batch unwound
