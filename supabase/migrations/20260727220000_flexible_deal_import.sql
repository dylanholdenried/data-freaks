-- Flexible deal import: allow blank customer last name; commit pending vs closed by completeness

alter table public.deals
  alter column customer_last_name drop not null;

comment on column public.deals.customer_last_name is
  'Customer last name. Nullable for pending deals with incomplete identity.';

-- Recreate import commit RPC: status from normalized payload, null-safe casts, optional roster/trades
