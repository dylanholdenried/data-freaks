-- Restrict deal-import SECURITY DEFINER RPCs to service_role only.
-- App commits/unwinds via createSupabaseServiceClient after requireAdminContext().
-- Also require platform admin when invoked with a user JWT (defense in depth).
...
begin
  if coalesce(auth.role(), '') <> 'service_role' and not public.is_platform_admin() then
    raise exception 'Not authorized to commit import batches'
      using errcode = '42501';
  end if;
...
begin
  if coalesce(auth.role(), '') <> 'service_role' and not public.is_platform_admin() then
    raise exception 'Not authorized to unwind import batches'
      using errcode = '42501';
  end if;
...
revoke all on function public.commit_deal_import_batch(uuid) from anon;
revoke all on function public.commit_deal_import_batch(uuid) from authenticated;
...
revoke all on function public.unwind_deal_import_batch(uuid) from anon;
revoke all on function public.unwind_deal_import_batch(uuid) from authenticated;
