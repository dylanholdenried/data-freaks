-- Bulk deal import staging tables + atomic commit RPC (platform admin)

create table if not exists public.deal_import_batches (
  ...
  constraint deal_import_batches_store_group_check
    check (true) -- enforced in app + commit RPC via stores.dealer_group_id
);

create index if not exists deal_import_batches_status_idx
  on public.deal_import_batches(status);
create index if not exists deal_import_batches_store_id_idx
  on public.deal_import_batches(store_id);
create index if not exists deal_import_batches_group_id_idx
  on public.deal_import_batches(dealer_group_id);

create table if not exists public.deal_import_rows (
  id uuid primary key default gen_random_uuid(),
  batch_id uuid not null references public.deal_import_batches(id) on delete cascade,
...
create index if not exists deal_import_rows_batch_id_idx
  on public.deal_import_rows(batch_id);

alter table public.deal_import_batches enable row level security;
alter table public.deal_import_rows enable row level security;

drop policy if exists deal_import_batches_platform_admin_all on public.deal_import_batches;
create policy deal_import_batches_platform_admin_all
  on public.deal_import_batches
  for all
  using (public.is_platform_admin())
  with check (public.is_platform_admin());

drop policy if exists deal_import_rows_platform_admin_all on public.deal_import_rows;
create policy deal_import_rows_platform_admin_all
  on public.deal_import_rows
  for all
  using (public.is_platform_admin())
  with check (public.is_platform_admin());

-- Atomically create missing roster rows + insert all closed deals for a valid batch.
-- Uses live app column names (entered_by, acquisition_source text, trades.year/make/..., deal_notes.note).
...
begin
  ...
  loop
    ...
    -- Resolve finance manager id (existing or just-created)
