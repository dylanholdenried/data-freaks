#!/usr/bin/env node
/**
 * Apply supabase/migrations/20260725140000_deal_import_batches.sql via Management API.
 * Requires: SUPABASE_ACCESS_TOKEN (Dashboard → Account → Access Tokens)
 * Optional: SUPABASE_PROJECT_REF (defaults from NEXT_PUBLIC_SUPABASE_URL in .env.local)
 *
 *   SUPABASE_ACCESS_TOKEN=sbp_... node scripts/apply-deal-import-migration.mjs
 */
import fs from "fs";
...
if (!res.ok) {
  console.error("Apply failed:", res.status, text.slice(0, 1000));
  process.exit(1);
}
