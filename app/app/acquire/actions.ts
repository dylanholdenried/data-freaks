...
export async function createAcquirePurchase(
  ...
  try {
    ...
    let dealerGroupId = ctx.dealerGroupId;
    ...
    if (storeId) {
      ...
      dealerGroupId = store.dealer_group_id;
    }
    ...
    if (payload.stock_number && payload.store_id) {
      try {
...
export async function updateAcquirePurchase(
  ...
  try {
    ...
    if (
      existing.store_id &&
      !(await assertStoreAccess(ctx.supabase, ctx.profile, existing.store_id))
    ) {
      return { ok: false, error: "No access to that store." };
    }
    ...
    const nextStoreId = formData.has("store_id")
      ? emptyToNull(formData.get("store_id"))
      : (existing.store_id as string | null);

    if (nextStoreId) {
      if (!(await assertStoreAccess(ctx.supabase, ctx.profile, nextStoreId))) {
        return { ok: false, error: "No access to that store." };
      }
      const { data: store } = await ctx.supabase
        .from("stores")
    ...
    if (patch.stock_number && nextStoreId) {
      try {
        const service = createSupabaseServiceClient();
        const { syncAcquireOverlaysForStoresLatest } = await import(
          "@/lib/acquire/sync-inventory"
        );
        await syncAcquireOverlaysForStoresLatest(service, [nextStoreId]);
      } catch (syncErr) {
